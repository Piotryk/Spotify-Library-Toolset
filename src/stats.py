...
def most_common_word_in_lyrics(songs, playlist_name):
    ...
    number_of_soungs_found = len(all_lyrics)
    if not all_lyrics:
        print(all_lyrics)
        return None
    ...
    with open(filename, 'w', encoding="utf-8") as f:
        ...
        for word in stop_words:
            f.write(f'{word}, ')
        f.write('\n')
        f.write(f'Analyzed lyrics for {number_of_soungs_found} songs:\n\n')
...
def artists_in_first_n_songs_compare(spotify, playlist_id_1, playlist_id_2, shuffle=False, fit=False):
    ...
    :param spotify: spitipy object
    :param playlist_id_1: id of 1st playlist
    :param playlist_id_2: id of 2nd playlist
    :param shuffle: True - shuffle artists positions. False - keep playlist
    :param fit: True - display fitted curve. False - do not fit curve
    :return: None
    """
...
def make_all_stats(spotify_data, playlist_id, renew_data=False):
    """
    Make all avalible stats for a playlist.
    :param spotify_data:   playlist id to make stats
    :param playlist_id:   playlist id to make stats
    :return: None
    """
    playlist_data = {}
    for playlist in spotify_data:
        ...
        print(f'Stats: Missing playlist data.')
        return None
...
def make_all_stats_data_based(spotify, playlist_id, renew_data=False):
    """
    WIP
    Make all avalible stats for a playlist.
    ...
    data_filename = 'data/stats_temp.pkl'
    if renew_data:
