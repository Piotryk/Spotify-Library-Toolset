...
def find_duplicaded_songs(spotify_data, tags, excluded_tags, use_exceptions=True):
    """
    Finds songs that are on more than one playlist.
    Found cases are saved to a file.

    :param spotify_data:         spotipy object
    :param tags:                 list of tags. Only playlists with provided tags will be checked
    :param excluded_tags:        list of tags. Playlists provided tags will be skipped
    :param use_exceptions:
    :return: None
    """
    ...
    for i, playlist in enumerate(spotify_data):
        ...
        if tags:
            ...
            excluded_tag_in_desc = any([tag in excluded_tags for tag in tags])
            if not tag_in_desc or excluded_tag_in_desc:     # If there is no tag skip playlist
                continue
...
def find_duplicated_artists(spotify_data, tags, excluded_tags, use_exceptions=True):
    """
    Finds songs that are on more than one playlist.
    Found cases are saved to a file.
...
def check_for_similar_names(spotify_data, tags, excluded_tags, use_exceptions=True):
    """
